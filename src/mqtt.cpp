...
void MQTT::begin(String host, int port, String username, String password, String configurationUrl) {
	...
	INFO("Connecting to %s:%d as client %s with password %s", host.c_str(), port, this->clientid.c_str(), password.c_str());
	this->host = host;
	this->port = port;
...
void MQTT::performAutoDiscovery() {
	...
	this->announceButton("startstop", "Start/Stop", "mdi:restart", [this]() { this->app->toggleActiveState(); });

	this->announceButton("next", "Next Title", "mdi:skip-next", [this]() { this->app->next(); });

	this->announceButton("previous", "Previous Title", "mdi:skip-previous", [this]() { this->app->previous(); });
