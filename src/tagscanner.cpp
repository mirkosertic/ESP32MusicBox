...
TagScanner::TagScanner(TwoWire *wire, uint8_t irq, uint8_t reset)
{
    ...
    {
        WARN("Data to write queue could not be created. Halt.");
        while (1)
        {
            delay(1000);
        }
...
void TagScanner::scan()
{
    ...
    if (tagscannersuccess)
    {
        ...
        if (tagscanneruidLength == 4)
        {
            ...
            TagData tagdata;
            int ret = xQueueReceive(this->dataToWrite, &tagdata, 0);
            if (ret == pdPASS)
            {
...
void TagScanner::begin(TagDetectedCallback callback, NoTagCallback noTagCallback)
{
    ...
    xTaskCreatePinnedToCore(scandispatcher, "NFC Scanner", 4096, this, 5, NULL, 1);
}
...
void TagScanner::write(uint8_t *userdata, uint8_t size)
{
    ...
    int ret = xQueueSend(this->dataToWrite, (void *)&tagdata, 0);
    if (ret == pdTRUE)
    {
